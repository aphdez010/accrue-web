      ...
      {loading ? (
        ...
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 24 }}>
            {[
              { label: 'Total Hours', val: Number(d.totalHours||0).toFixed(1), color: 'var(--ink)' },
              { label: 'Supervision %', val: Number(d.supervisionPct||0).toFixed(1)+'%', color: d.supervisionMet ? 'var(--spruce)' : 'var(--amber)' },
              { label: 'Restricted %', val: Number(d.restrictedPct||0).toFixed(1)+'%', color: d.restrictedMet ? 'var(--spruce)' : 'var(--amber)' },
            ].map(s => (
              <div key={s.label} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px 28px' }}>
                <p style={{ fontFamily: 'var(--mono)', fontSize: 10, textTransform: 'uppercase', color: 'var(--muted)', marginBottom: 4 }}>{s.label}</p>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '28px 32px', marginBottom: 24 }}>
            <p style={{ fontFamily: 'var(--mono)', fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--muted)', marginBottom: 20 }}>BACB Requirements</p>
            {[
              { label: 'Supervision >= 5% of hours', pass: d.supervisionMet, val: Number(d.supervisionPct||0).toFixed(1)+'%' },
              { label: 'Restricted hours <= 50% of total', pass: d.restrictedMet, val: Number(d.restrictedPct||0).toFixed(1)+'%' },
            ].map((r, i, arr) => (
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '28px 32px' }}>
            <p style={{ fontFamily: 'var(--mono)', fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--muted)', marginBottom: 20 }}>Hours Breakdown</p>
            {[['Unrestricted', d.unrestricted],['Restricted', d.restricted],['Supervised', d.supervisedHours],['Total', d.totalHours]].map(([label, val], i, arr) => (
