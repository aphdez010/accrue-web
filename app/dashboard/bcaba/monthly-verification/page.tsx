...
const TRAINEE_ID = 1; // TEMP: matches convention used elsewhere until a trainee selector exists
...
export default function MonthlyVerificationPage() {
  ...
  const load = () => {
    setLoading(true);
    get('/bcaba-monthly-verification?traineeId=' + TRAINEE_ID).then((r: any) => {
  ...
  async function draftMonthly() {
    ...
    try {
      const r: any = await post('/bcaba-monthly-verification/draft', { traineeId: TRAINEE_ID, monthYear });
