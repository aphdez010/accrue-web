...
export default function FieldworkPage() {
  ...
  async function load() {
    try {
      ...
      const list: FieldworkEntry[] = res?.entries ?? res ?? [];
      setEntries(list);
  ...
  return (
    <div style={{ padding: '40px', maxWidth: 880 }}>
      ...
      <div style={{ ...card, marginBottom: 24 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
          ...
          <div>
            ...
            <input type="number" step="0.25" min="0" placeholder="e.g. 2.5"
              value={form.hours}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
              <div onClick={() => setForm(f => ({ ...f, is_supervised: !f.is_supervised }))}
                style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <button onClick={handleSubmit}
            ...
            style={{
              ...
              padding: '11px 28px', fontFamily: 'var(--mono)', fontSize: 12,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
        <div style={card}>
          <p style={{ fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: 4 }}>Total Hours</p>
          <p style={{ fontFamily: 'var(--display)', fontSize: 32, fontWeight: 600, color: 'var(--ink)', margin: 0, lineHeight: 1 }}>{totalHours.toFixed(1)}</p>
        </div>
        <div style={card}>
          <p style={{ fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: 4 }}>Entries</p>
          <p style={{ fontFamily: 'var(--display)', fontSize: 32, fontWeight: 600, color: 'var(--ink)', margin: 0, lineHeight: 1 }}>{entries.length}</p>
        </div>
      </div>
      ...
      <div style={card}>
        ...
        {loading ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>{['Date', 'Type', 'Hours', 'Supervised', 'Notes'].map(h => <th key={h} style={thStyle}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {entries.map((e, i) => (
                <tr key={e.id} style={{ borderBottom: i < entries.length - 1 ? '1px solid var(--border)' : 'none' }}>
                  ...
                  <td style={{ ...tdStyle, fontFamily: 'var(--display)', fontSize: 14, fontWeight: 500 }}>{Number(e.hours).toFixed(1)}</td>
                  <td style={tdStyle}>
                    <span style={{
