                  ...
                  <>
                    ...
                    <div style={{ display: 'flex', gap: 2, flexWrap: 'wrap' as const, justifyContent: 'center' }}>
                      ...
                      {Array.from(dayData.supFormats).map((f: string) => (
                        <span key={f} title={f === 'individual' ? 'Individual' : 'Group'} style={{ fontFamily: 'var(--mono)', fontSize: 8, lineHeight: 1, padding: '1px 3px', borderRadius: 3, background: 'rgba(45,143,214,0.12)', color: 'var(--sky)' }}>
                          {f === 'individual' ? 'I' : 'G'}
                        </span>
                      ))}
                    </div>
